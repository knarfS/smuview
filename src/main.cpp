...
int selftest()
{
	...
	qInfo().noquote() << py_test_msg;
	qInfo().noquote() << QString("Embedded sys.version: %1").arg(py_sys_version);
	qInfo().noquote() << QString("Embedded sys.executable: %1").arg(py_sys_executable);
	qInfo().noquote() << QString("Embedded sys.prefix: %1").arg(py_sys_prefix);
	qInfo().noquote() << QString("Embedded sys.base_prefix: %1").arg(py_sys_base_prefix);
	qInfo().noquote() << QString("Embedded sys.path: %1").arg(py_sys_path);
	qInfo().noquote() << QString("Embedded os.__file__: %1").arg(py_os_module_file);
	...
	qInfo().noquote() << QString("SV_MANUAL_DIR: %1").arg(SV_MANUAL_DIR);
