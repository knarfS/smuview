...
echo "TOOLCHAIN_TRIPLET ${TOOLCHAIN_TRIPLET:?TOOLCHAIN_TRIPLET is not set}"
...
cd libserialport
./autogen.sh
./configure --host="${TOOLCHAIN_TRIPLET}" --prefix="${INSTALL_DIR}" CPPFLAGS=-D__printf__=__gnu_printf__ --disable-shared --enable-static
make -j"$(nproc)"
make install
cd ..
...
cd libsigrok
./autogen.sh
./configure --host="${TOOLCHAIN_TRIPLET}" --prefix="${INSTALL_DIR}" CPPFLAGS=-D__printf__=__gnu_printf__ --disable-shared --enable-static --disable-java --disable-ruby
make -j"$(nproc)"
make install
cd ..
