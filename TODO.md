...
- Remove relative paths (`./smuview/build/` and `../smuview/`) in `macos/build_dmg.sh` and pass in `${SV_SOURCE_DIR}` and `${SV_BUILD_DIR}`
- Remove hard coded python version in installers (nsis, build-appimage.sh)
- Windows verification window title not reliable
- Document RPM gerneration prcess.
...
- Save all data (gnuplot, octave, ...)
	- Move CSV logic out of `signalsavedialog.cpp`!
	- add a saveutil.cpp
- Configurable:
...
- contrib/smuview_cross.nsi.in -> packaging/mxe/smuview.nsi and pass values with `makensis -DSV_VERSION_STRING=0.0.7-xxx -DPROJECT_SOURCE_DIR=... ...` and check with
  ```
  !ifndef SV_VERSION_STRING
    !error "SV_VERSION_STRING was not specified!"
  !endif
  # ...
